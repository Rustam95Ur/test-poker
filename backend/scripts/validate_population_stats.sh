...
run_query "stat-001 | Flop bet frequency | population" "
SELECT
...
run_query "stat-002 | Triple barrel frequency | population" "
SELECT
  (SELECT coalesce(sum(cnt),0) FROM events_agg
   WHERE source_type='population'
...
run_query "stat-003 | Delayed river bet frequency | population" "
SELECT
  (SELECT coalesce(sum(cnt),0) FROM events_agg
   WHERE source_type='population'
     AND spot='SRP' AND formation='BB_SB' AND position='OOP' AND role='PFR' AND street='river') AS context_cnt,
